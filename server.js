...
app.post('/api/trades', (req,res)=>res.status(501).json({error:'Trading adapter not configured',message:'Live execution is disabled until the authenticated Deriv trading adapter is configured and tested.'}));
app.post('/api/bot', (req,res)=>res.status(501).json({error:'Bot adapter not configured',message:'The free bot interface is ready, but real execution is disabled until the controlled Deriv bot adapter is configured.'}));
app.get('/api/account', (req,res)=>{const s=sessions.get(req.cookies?.linus_session);if(!s||Date.now()>=s.expiresAt)return res.status(401).json({authenticated:false});return res.status(501).json({error:'Account adapter not configured'});});
...
app.get('/marketplace', (req,res)=>res.sendFile(path.join(PUBLIC_DIR,'pages','marketplace.html')));
app.get('/marketplace.html', (req,res)=>res.sendFile(path.join(PUBLIC_DIR,'pages','marketplace.html')));
app.get('/course', (req,res)=>res.sendFile(path.join(PUBLIC_DIR,'pages','course.html')));
app.get('/course.html', (req,res)=>res.sendFile(path.join(PUBLIC_DIR,'pages','course.html')));
app.get('/signals', (req,res)=>res.sendFile(path.join(PUBLIC_DIR,'pages','signals.html')));
app.get('/manual', (req,res)=>res.sendFile(path.join(PUBLIC_DIR,'pages','manual.html')));
app.get('/builder', (req,res)=>res.sendFile(path.join(PUBLIC_DIR,'pages','builder.html')));
