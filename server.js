...
app.get('/api/market/symbols', async (req, res) => { try { const productType = String(req.query.product_type || 'all'); const data = await openDerivPublic({ active_symbols: 'full', product_type: productType }); res.set('Cache-Control', 'no-store').json({ msg_type: data.msg_type, keys: Object.keys(data), active_symbols: data.active_symbols || [], error: data.error || null }); } catch (error) { res.status(502).json({ error: 'Market symbols unavailable', message: error.message }); } });
