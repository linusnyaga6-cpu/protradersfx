...
router.post("/deriv/execute", async (req, res) => {
  ...
  try {
    ...
    const buyPrice = finiteNumber(contract.buy_price) ?? finiteNumber(buy.buy_price) ?? finiteNumber(proposal.ask_price) ?? body.amount as number;
    const payout = finiteNumber(contract.payout);
    const profit = finiteNumber(contract.profit)
